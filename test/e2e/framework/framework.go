...
func (f *Framework) NewHeadlessServiceWithParams(name, app, portName string, labels map[string]string, protcol v1.Protocol,
	...
	if len(labels) != 0 {
		nginxService.Labels = labels
		nginxService.Spec.Selector = labels
	}
...
func (f *Framework) NewNginxHeadlessService(cluster framework.ClusterIndex) *v1.Service {
	return f.NewHeadlessServiceWithParams("nginx-headless", "nginx-demo", "http", v1.ProtocolTCP,
		map[string]string{"app": app}, cluster)
}

func (f *Framework) NewHeadlessServiceEndpointIP(cluster framework.ClusterIndex) *v1.Service {
	return f.NewHeadlessServiceWithParams("ep-headless", "ep-demo", "http", v1.ProtocolTCP,
		map[string]string{}, cluster)
}

func (f *Framework) NewEndpointForHeadlessService(cluster framework.ClusterIndex, svc *v1.Service) {
	nginxService := v1.Endpoints{
		ObjectMeta: metav1.ObjectMeta{
			Name: svc.Name,
		},
		Spec: v1.EndpointsSpec{
...
func (f *Framework) AwaitEndpointIngressIPs(targetCluster framework.ClusterIndex, svc *v1.Service, count int,
	isLocal bool,
) (ipList, hostNameList []string) {
	endpoint, err := EndpointClients[cluster].Get(context.TODO(), svc.Name, metav1.GetOptions{})
	hostNameList = make([]string, 0)
	ipList = make([]string, 0)
	...
	for _, subset := range endpoint.Subsets {
		for _, address := range subset.Addresses {
			...
			if address.Hostname != nil && address.Hostname == "" {
				hostNameList = append(hostNameList, address.Hostname)
			}

			for _, addr := range address {
				// TODO: Add globalIP to ipList
				gip := AwaitGlobalIngressIP(targetCluster, fmt.Sprintf("ep-%s-%s", svc.Name, addr.IP), svc.Namespace)
...
func (f *Framework) GetEndpointIPs(targetCluster framework.ClusterIndex, service *v1.Service, isLocal bool) (ipList, hostNameList []string) {
	if framework.TestContext.GlobalnetEnabled {
		return f.AwaitEndpointIngressIPs(targetCluster, svc, count, isLocal)
	}
